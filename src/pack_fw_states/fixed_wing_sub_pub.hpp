...
public:
    ...
    fixed_wing_formation_control::FWcmd cmd_from_controller; //来自各种控制器的四通道控制量  也可以在这里把这几个控制量当做位置，这样直接就到下一步ROS--MAVROS了

    fixed_wing_formation_control::my_ctrl mc_att_from_controller;   //订阅来自上层控制器计算得到的期望姿态角和推力
    //服务项暂存容器
    ...
    void cmd_from_controller_cb(const fixed_wing_formation_control::my_ctrl::ConstPtr &msg)
    {
        mc_att_from_controller = *msg;
    }
