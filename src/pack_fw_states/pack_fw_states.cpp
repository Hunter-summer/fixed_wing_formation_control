...
void PACK_FW_STATES::ros_sub_and_pub() {
  ...
  //发布期望位置   因为这个没有再那里面进行更新，所以  这才是发布的MAVROS消息  
...
void PACK_FW_STATES::pack_fw_states() {
  ...
  /*fixed_wing_sub_pub.fw_states_form_mavros.gxround_speed=sqrt(fixed_wing_sub_pub.fw_states_form_mavros.global_vel_x*fixed_wing_sub_pub.fw_states_form_mavros.global_vel_x
  +fixed_wing_sub_pub.fw_states_form_mavros.global_vel_y*fixed_wing_sub_pub.fw_states_form_mavros.global_vel_y
  +fixed_wing_sub_pub.fw_states_form_mavros.global_vel_z*fixed_wing_sub_pub.fw_states_form_mavros.global_vel_z);
*/
...
//期望姿态角
 fixed_wing_sub_pub.fw_states_form_mavros.roll_traget_angle =
      fixed_wing_sub_pub.cmd_from_controller.roll_angle_sp;
 fixed_wing_sub_pub.fw_states_form_mavros.pitch_traget_angle=
    fixed_wing_sub_pub.cmd_from_controller.pitch_angle_sp;
  fixed_wing_sub_pub.fw_states_form_mavros.yaw_traget_angle=
      fixed_wing_sub_pub.cmd_from_controller.yaw_angle_sp;
...
//   //将期望值转换一下坐标系，并转化为四元数   固定翼
  float angle[3], quat[4];

  angle[0] = fixed_wing_sub_pub.cmd_from_controller.roll_angle_sp;
  angle[1] = -fixed_wing_sub_pub.cmd_from_controller.pitch_angle_sp; //-1~1 1->90度  -1->-90
  angle[2] =-fixed_wing_sub_pub.cmd_from_controller.yaw_angle_sp + deg_2_rad(90.0);
//cout<<"angle[2]:"<<angle[2]<<endl;
//cout<<"-fixed_wing_sub_pub.cmd_from_controller.yaw_angle_sp"<<-fixed_wing_sub_pub.cmd_from_controller.yaw_angle_sp<<endl;
  cnt++;
  euler_2_quaternion(angle, quat);
  fixed_wing_sub_pub.att_sp.type_mask =0b0000000; // 1+2+4+64+128 body.rate_x,body.rate_y,body.rate_z thrust..  1表示屏蔽
  ...
  //fixed_wing_sub_pub.att_sp.body_rate
  fixed_wing_local_att_sp_pub.publish(fixed_wing_sub_pub.att_sp);

}


void PACK_FW_STATES::msg_to_mavros_iris() {

//将期望位置发送给PX4  旋翼   因为不直接给位置控制  还是应该选择给期望姿态到内环  需要进行外环位置的PID控制
  fixed_wing_sub_pub.local_pos_sp.type_mask = 0b0000000000000000;
  fixed_wing_sub_pub.local_pos_sp.coordinate_frame=1;
  fixed_wing_sub_pub.local_pos_sp.position.x  =  0;//fixed_wing_sub_pub.mc_att_from_controller.x_sp;
  fixed_wing_sub_pub.local_pos_sp.position.y = 0;//fixed_wing_sub_pub.mc_att_from_controller.y_sp;
  fixed_wing_sub_pub.local_pos_sp.position.z = 5;//fixed_wing_sub_pub.mc_att_from_controller.z_sp;
  fixed_wing_sub_pub.local_pos_sp.velocity.x =  0;//fixed_wing_sub_pub.mc_att_from_controller.vx;
  fixed_wing_sub_pub.local_pos_sp.velocity.y =  0;//fixed_wing_sub_pub.mc_att_from_controller.vy;
  fixed_wing_sub_pub.local_pos_sp.velocity.z =  0;//fixed_wing_sub_pub.mc_att_from_controller.vz;
  fixed_wing_sub_pub.local_pos_sp.acceleration_or_force.x=0;//fixed_wing_sub_pub.mc_att_from_controller.ax;
  fixed_wing_sub_pub.local_pos_sp.acceleration_or_force.y=0;//fixed_wing_sub_pub.mc_att_from_controller.ay;
  fixed_wing_sub_pub.local_pos_sp.acceleration_or_force.z=0;//fixed_wing_sub_pub.mc_att_from_controller.az;
  
 fixed_wing_sub_pub.local_pos_sp.yaw = 0;//fixed_wing_sub_pub.mc_att_from_controller.yaw_sp;
 fixed_wing_sub_pub.local_pos_sp.yaw_rate = 0;//fixed_wing_sub_pub.mc_att_from_controller.yaw_sp;

  cout<<"local_pos_sp_x:"<<fixed_wing_sub_pub.local_pos_sp.position.x<<endl;
  cout<<"local_pos_sp_y:"<<fixed_wing_sub_pub.local_pos_sp.position.y<<endl;
  cout<<"local_pos_sp_z:"<<fixed_wing_sub_pub.local_pos_sp.position.z<<endl;
  PACK_FW_STATES_INFO("local_pos_sp_x1:" << fixed_wing_sub_pub.local_pos_sp.position.x);
  PACK_FW_STATES_INFO("local_pos_sp_y1:" << fixed_wing_sub_pub.local_pos_sp.position.y);
    PACK_FW_STATES_INFO("local_pos_sp_z1:" << fixed_wing_sub_pub.local_pos_sp.position.z);
  fixed_wing_local_pos_sp_pub.publish(fixed_wing_sub_pub.local_pos_sp);


  // //将期望值转换一下坐标系，并转化为四元数  旋翼
  // float angle[3], quat[4];
  // //需要测试正负号
  // angle[0] = fixed_wing_sub_pub.mc_att_from_controller.roll_sp;
  // angle[1] = -fixed_wing_sub_pub.mc_att_from_controller.pitch_sp; //-1~1 1->90度  -1->-90
  // angle[2] =-fixed_wing_sub_pub.mc_att_from_controller.yaw_sp + deg_2_rad(90.0);

  // euler_2_quaternion(angle, quat);
  // fixed_wing_sub_pub.att_sp.type_mask =0b00000000; // 1+2+4+64+128 body.rate_x,body.rate_y,body.rate_z thrust..  1表示屏蔽
  // fixed_wing_sub_pub.att_sp.orientation.w = quat[0];
  // fixed_wing_sub_pub.att_sp.orientation.x = quat[1];
  // fixed_wing_sub_pub.att_sp.orientation.y = quat[2];
  // fixed_wing_sub_pub.att_sp.orientation.z = quat[3];
  // fixed_wing_sub_pub.att_sp.thrust = fixed_wing_sub_pub.mc_att_from_controller.thrust_sp;
  // fixed_wing_local_att_sp_pub.publish(fixed_wing_sub_pub.att_sp);
...
void PACK_FW_STATES::run(int argc, char **argv) {
  ...
  while (ros::ok()) {
    ...
    pack_fw_states();
    PACK_FW_STATES_INFO("将ROS_Msgs转换为MAVROS消息!!!!");
    msg_to_mavros();
    //msg_to_mavros_iris();
