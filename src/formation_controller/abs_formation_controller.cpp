...
void ABS_FORMATION_CONTROLLER::control_formation()
{
  ...
  /**
    * 1. 根据队形要求，计算出从机期望位置在领机航迹坐标系下的位置，然后再到->GPS位置（期望经纬高）；
    * 在此之中注意领机航迹方向的选择。
    *   a. 计算领机机头方向
    *   b. 计算从机期望位置的gps位置
    */
...
//cout<<"fw_error.Yk"<<fw_error.PYk <<endl;
 ...
 // fw_error.led_fol_vxk = fw_dir_unit * led_fol_vel_error;                         /* 沿地速方向 */
  ...
  fw_error.led_fol_vyk = fw_dir_unit ^ led_fol_vel_error;                         /* 垂直地速方向*/
 ...
 // else{
              ...
              /*编队控制器   根据误差进行编队*/
            float k1n_pitch = -0.008;  //-0.02    -0.008;
            float k2n_pitch = -0.08; //-0.05   -0.08;
            float k_nm_pitch = 1;
            float p2_pitch = 0.25;
            float p3_pitch = 0.15;
            float ctrl_pitch = 0;  //>=1 adaptive    <1 nominal  
            cout<<"****************************X轴**************************"<<endl;
           my_ctl.out_controller(-fw_error.PXk,-fw_error.led_fol_vxk,(leader_states_f.global_vel_y-leader_vel_prev)/_dt,k1n_pitch,k2n_pitch,k_nm_pitch,p2_pitch,p3_pitch,_dt,ctrl_pitch);//-0.05 -0.2   //-0.01 -0.1
            ...
            cout<<"my_ctrl的x轴ax:"<<my_ctl.get_output()<<endl;
            cout<<"my_ctrl的x轴输出量v_k:"<<my_ctl.get_output_spd()<<endl;
...
/*  pitch_ctl.init_pid(0.05,0.01,0);
 ...
 _cmd.thrust=thrust_ctl.pid_anti_saturated(fw_sp.air_speed-fw_states_f.http://wiki.ros.org/cn/ROS/Tutorials#A.2BUh1.2Bp2VZegs-
...
//*****************控制器加入********************************//
        if(rest_my_contoller)
              ...
              /*编队控制器   根据误差进行编队*/
            float k1n_roll = -0.010; //   -0.015
            float k2n_roll = -0.03;  //   -0.04
            float k_nm_roll = 1;
            float p2_roll = 0.25;
            float p3_roll = 0.15;
            float ctrl_roll = 0;  //>=1 adaptive    <1 nominal  
            float mix_error{0.0};
            float mix_kp{0.0},mix_eta_kp{0.0},mix_eta_pre{0.0};
            float mix_error_v{0.0},mix_eta_kd{0.0},mix_kd{0.0};
            cout<<"****************************Y轴**************************"<<endl;
            //  cout<<"fw_error_pyk:"<<fw_error.PYk<<endl;
            //  cout<<"fw_error_led_fol_vyk"<<fw_error.led_fol_vyk<<endl;
            
            /* 位置与角度误差  不同的角度不一样  分段 */
            if (fw_error.led_fol_eta <= deg_2_rad(45.0) &&
                fw_error.led_fol_eta >= deg_2_rad(-45.0)) {
              mix_kp = 0.5;mix_eta_kp=0.5;mix_kd = 0.8;mix_eta_kd = 0.2;
            } else {
              mix_kp = 0.0;mix_eta_kp=0.5;mix_kd = 0.2;mix_eta_kd = 4.0;

            }
            cout<<"mix_kp:"<<mix_kp<<"mix_eta:"<<mix_eta_kp<<endl;
            mix_error = mix_kp*fw_error.PYk + mix_eta_kp * fw_error.led_fol_eta;
            mix_error_v = mix_kd * fw_error.led_fol_vyk  + mix_eta_kd * (fw_error.led_fol_eta - mix_eta_pre/_dt); //+eta

            my_roll_ctrl.out_controller(-mix_error,-mix_error_v,(leader_states_f.global_vel_x-leader_vel_prev_1)/_dt,k1n_roll,k2n_roll,k_nm_roll,p2_roll,p3_roll,_dt,ctrl_roll);//-0.05 -0.2   //-0.01 -0.1
            mix_eta_pre = fw_error.led_fol_eta;
            //控制量怎么存起来   
             /*计算控制部分    这个也可以试一下   不一定能用，得调试  */ 
             //x 指的是航迹坐标系xk， y指的是航迹坐标系yk
              //controller();
            //cout<<"Controller的y轴输出量:"<<U_r<<endl;
             //cout<<"my_roll_ctrl的y轴ay:"<<my_roll_ctrl.get_output()<<endl;
            ...
            cout<<"my roll:"<<roll_cmd<<endl;
            //roll_cmd = roll_cmd_ori;
            cout<<"roll origin:"<<roll_cmd_ori<<endl;
//控制器结束
...
//*****获取cmd期望roll*****//
 // _cmd.roll = roll_cmd;
  _cmd.roll = fw_error.PXk;

}

void ABS_FORMATION_CONTROLLER::get_adaptive_ctrl(float *a_x,float *a_y)
{
  cout<<"进入gat_adaptive_ctrl函数"<<endl;
  *a_x = my_ctl.get_output();
  *a_y = my_roll_ctrl.get_output();
}

// void ABS_FORMATION_CONTROLLER::get_adaptive_roll_ctrl(float *a_y)
// {
//   *a_y = my_ctl.get_output();
// }